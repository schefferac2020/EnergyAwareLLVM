...
class Solver:
    ...
    def codetostr(self,block):
        ...
        if isinstance(block,ir.BasicBlock):
            return self.s+res
        return res
    def getenergy_(self,block):
        '''Get energy from JSON file'''
        if isinstance(block,ir.Function):
            return 0
        if self.mapping:
...
class PubsSolver(Solver):
    ...
    def vartostr(self,var):
        ...
        assert isinstance(var,ir.Instruction),var
        name='V'+var.name
...
class LLVMElem:
    def __init__(self,_):
        self._=_
        assert not isinstance(_,ir.ConstantInt)
...
class Inferencer:
    ...
    def getarg(self,block,var):
        assert isinstance(block,ir.BasicBlock),block
        ga=lambda var: self.getarg(block,var)
        if isinstance(var,ir.Argument):
            return 'arg',var
        if isinstance(var,ir.ConstantInt):
            return 'const',str(tosigned(var.z_ext_value))
        if isinstance(var,ir.ConstantFP):
            return '?',None
        if isinstance(var,ir.ConstantExpr):
            return '?',None
        if isinstance(var,ir.ConstantPointerNull):
            return '?',None
        if isinstance(var,ir.UndefValue):
            return 'const',0
        assert isinstance(var,ir.Instruction),str(var)
    ...
    def genargs(self,block):
        ...
        for r in res:
            assert not isinstance(r._,ir.ConstantInt)
        return res
    ...
    def get_vars_from_arg(self,out,arg):
        ...
        if op in {'phi','var','arg'}:
            ...
            if isinstance(rst[0].type,ir.IntegerType) and rst[0].type.width>1:
                    ...
                    for var in set(self.vars[succ]):
                        if getattr(var._,'opcode_name',None)=='phi' and LLVMElem(var._.basic_block)==succ:
                            for n in range(var._.operand_count):
                                ...
                                if not isinstance(val, (ir.Argument, ir.Instruction)):
                                    continue
...
def main(_,bitcodefile,jsonfile=None):
    print("Opening ll file: ", bitcodefile) # TODO: Note that this is not actually a bitcode file
    with open(bitcodefile,'r') as f:
        the_ir = str(f.read())
...
#    solver=NumericSolver()
    ...
    for i, fn in enumerate(mod_bc.functions):
        num_basic_blocks = len(list(fn.blocks))
        print(f"This function has {num_basic_blocks} basic blocks")
        # infer=Inferencer(solver,fn,'f%d'%i) #TODO: uncomment this
